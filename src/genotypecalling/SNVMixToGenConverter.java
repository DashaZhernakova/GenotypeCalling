...
public class SNVMixToGenConverter {
    ...
    public void parse(String fileListPath, String outPrefix, String conversionVCFFile) throws IOException {
    ...
    private void makeMatrix() throws IOException{
        //get all diallelic SNPs passing p-value threshold
        getAllSNPs();
        ...
        for (String sampleFile : files){
            ...
            while ( (els = sFile.readLineElems(TextFile.tab)) != null ){
                ...
                if (prob[gen - 1] >= p_threshold){
                    if (! snpSet.contains(els[0])) //skip if triallelic
                        continue;
